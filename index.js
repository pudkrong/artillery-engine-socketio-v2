  ...
  engineUtil.captureOrMatch(response, fauxResponse, context, function (err, result) {
    ...
    if (result !== null) {
      ...
      if (failedMatches.length > 0) {
        debug(failedMatches);
        // TONOTE::PUD data is arguments; response we get from socket.io
        // Here we don't know the request spec, so we cannot make error more sense
        // that where the error come from. If you need that we have to pass `requestSpec`
        // Which makes the function look bad
        // Amity will respond error as
        // {
        //   "status": "error",
        //   "message": "Parameters validation error!",
        //   "code": 500000
        // }
        const errSocket = data['0'];
        if (errSocket) {
          ee.emit('error', `${context.vars['emit'].channel}.code.${errSocket.code}`);
          return callback(new Error(`Channel error as ${errSocket.code}`), context);
        } else {
    ...
    let endCallback = function (err, context, needEmit) {
      ...
      if (isAcknowledgeRequired(requestSpec)) {
        ...
        let ackCallback = function () {
          ...
          context.vars['emit'] = requestSpec.emit;
        ...
        if (needEmit) {
          ...
          // TONOTE::PUD We add ack timeout here
          // If we don't get a response within the timeout, fire an error
          let waitTime = self.config.timeout || 10;
          waitTime *= 1000;
          timeout = setTimeout(function responseTimeout () {
            let err = `${requestSpec.emit.channel}.response timeout`;
            ee.emit('error', err);
            return callback(err, context);
          }, waitTime);
        } else {
